...
\code{MappedContainer}.  Each must be a \textit{sequence container}
(\S26.2.3).\\
...
Because of this, \code{value_type} is \code{pair<const Key &, T &>}, and both
\code{iterator} and \code{const_iterator} must be proxy iterators.  Note that
...
For instance, the result of \code{extract()} is no longer something that you
...
\begin{itemize}
  \item Several new constructors have been added that take objects of the
    \code{KeyContainer} and \code{MappedContainer} types.  These members must
...
\code{uses_allocator_v<KeyContainer, KeyAlloc>} and
