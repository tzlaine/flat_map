...
themselves. \tcode{flat_set} supports input iterators that meet the
...
reversible container ([container.rev.reqmts]).  \tcode{flat_set} satisfies
...
instantiate \tcode{flat_set}. In particular, \tcode{vector} ([vector]) and
...
\begin{itemdescr}
\pnum
\effects Constructs an empty \tcode{c}, inserts elements from \tcode{R}, value-initializes
\tcode{compare}, sorts the range \range{begin()}{end()} with respect to
\tcode{compare}.
...
keys themselves. \tcode{flat_multiset} supports input iterators that meet
the \oldconcept{InputIterator} requirements and model model
...
reversible container ([container.rev.reqmts]).  \tcode{flat_multiset} satisfies
the requirements of an associative container ([associative.reqmts]), except
that:
\begin{itemize}
\item it does not meet the requirements related to node handles ([container.node.overview]),
...
\pnum
Any sequence container supporting random access iteration can be used to
instantiate \tcode{flat_multiset}. In particular, \tcode{vector} ([vector])
...
\begin{addedblock}
\pnum
For each of \tcode{flat_set} and \tcode{flat_multiset}, the library provides
