...
a \tcode{sorted_unique_t} argument with a container or containers that is not
sorted with respect to \tcode{key_Comp()}, or that contains equal elements, is
undefined.
...
\pnum
The effect of calling a constructor or member function that takes
a \tcode{sorted_equivalent_t} argument with a container or containers that are
not sorted with respect to \tcode{key_comp()} is undefined.
