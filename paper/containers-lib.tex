...
[\textit{Example:} A call to \tcode{flat_map<Key, T>::insert} could invalidate all iterators to
the \tcode{flat_map}.]
\end{addedblock}
...
namespace std {
  ...
  template <class Key, class T, class Compare,
            class KeyContainer,
            class MappedContainer, class Alloc>
...
\pnum If an exception is thrown during any operation on \tcode{flat_map}, that
operation may clear the \tcode{flat_map}'s adapted containers to maintain
these invariants.

\pnum
Any sequence container supporting random access iteration can be used to
instantiate \tcode{flat_map}. In particular, \tcode{vector} ([vector]) and
\tcode{deque} ([deque]) can be used.  \begin{note}\tcode{vector<bool>} is
...
\tcode{value_comp()}; and finally and finally erases the duplicate elements
as if by:
\begin{codeblock}
...
\pnum
\effects
Equivalent to: \tcode{return min<size_type>(c.keys.max_size(), c.values.max_size());}
\end{itemdescr}
...
\begin{codeblock}
auto key_it = std::upper_bound(std::begin(c.keys), std::end(c.keys), t.first, compare);
...
pre-existing elements into a single sorted range; and finally and finally
...
\begin{codeblock}
auto key_it = std::upper_bound(std::begin(c.keys), std::end(c.keys), k, compare);
auto value_it = std::begin(c.values) + distance(std::begin(c.keys), key_it);
...
\pnum
Any sequence container supporting random access iteration can be used to
instantiate \tcode{flat_multimap}. In particular, \tcode{vector} ([vector])
and \tcode{deque} ([deque]) can be used.  \begin{note}\tcode{vector<bool>}
