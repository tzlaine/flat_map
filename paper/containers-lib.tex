...
\pnum
\constraints
\grammarterm{Qualified-id} \tcode{Compare::is_transparent} is valid and denotes a type.
...
\begin{itemdescr}
\pnum
\constraints \grammarterm{Qualified-id} \tcode{Compare::is_transparent} is valid and denotes a type.
...
\begin{itemdescr}
\pnum
\constraints \grammarterm{Qualified-id} \tcode{Compare::is_transparent} is valid
and denotes a type.  \tcode{is_constructible_v<key_type, K\&\&>} is
\tcode{true}.  \tcode{is_constructible_v<mapped_type, Args\&\&...>} is
\tcode{true}.  For the first overload, \tcode{is_convertible_v<K\&\&, const_iterator>}
and \tcode{is_convertible_v<K\&\&, iterator>} are both \tcode{false}.

\pnum
\expects The conversion from \tcode{k} into \tcode{key_type} constructs an
object \tcode{u}, for which \tcode{find(k)} == \tcode{find(u)} is true.
...
\constraints \grammarterm{Qualified-id} \tcode{Compare::is_transparent} is valid
and denotes a type.  \tcode{is_constructible_v<key_type, K\&\&>} is
\tcode{true}.  \tcode{is_assignable_v<mapped_type\&, M>}
is \tcode{true}, and \tcode{is_constructible_v<mapped_type, M\&\&>}
is \tcode{true}.
