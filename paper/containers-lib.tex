...
\pnum
A \tcode{flat_set} meets all of the requirements for a container
...
is restored.  \begin{note}This may result in the \tcode{flat_set}'s being
emptied.\end{note}
...
a \tcode{sorted_unique_t} argument with a range that is not sorted with
respect to \tcode{key_comp()}, or that contains equal elements, is
undefined.
...
\tcode{compare}, and finally erases all but the first element from each group of
...
\begin{itemdescr}
\pnum
\constraints \tcode{uses_allocator_v<container_type, Allocator>} is \tcode{true}.
...
\pnum
\effects Equivalent to \tcode{flat_set(s, cont)}, except that \tcode{c}
is constructed with uses-allocator construction
([allocator.uses.construction]).
...
\pnum
\expects The conversion from \tcode{x} into \tcode{value_type} constructs an
object \tcode{u}, for which \tcode{find(x) == find(u)} is true.

\pnum
\effects
If the set already contains an element equivalent to \tcode{x}, \tcode{*this}
and \tcode{x} is unchanged.  Otherwise, inserts a new element as if
by: \tcode{return emplace(std::forward<K>(x));}.
...
iterator points to the element whose key is equivalent to \tcode{k}.
\end{itemdescr}
...
\pnum \ensures \tcode{*this} is emptied, even if the function exits via an exception.
\end{itemdescr}
...
\pnum
A \tcode{flat_multiset} meets all of the requirements for a container
([container.reqmts]) and for a reversible container ([container.rev.reqmts]),
...
\pnum
A \tcode{flat_multiset} maintains the invariant that the keys are sorted with
respect to the comparison object.
...
the \tcode{flat_multiset}'s being emptied.\end{note}
...
\pnum
The effect of calling a constructor or member function that takes
a \tcode{sorted_equivalent_t} argument with a range that is not sorted with
respect to \tcode{key_comp()} is undefined.
...
\pnum
\effects Equivalent to \tcode{flat_multiset(s, cont)}, except that \tcode{c}
is constructed with uses-allocator construction
([allocator.uses.construction]).
