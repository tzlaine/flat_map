...
\pnum
Any sequence container ([sequence.reqmts]) \tcode{C} supporting random access
iteration can be used to instantiate \tcode{flat_map}, as long
...
\pnum
Any sequence container ([sequence.reqmts]) \tcode{C} supporting random access
iteration can be used to instantiate \tcode{flat_multimap}, as long
as \tcode{C::size()} and \tcode{C::max_size()} do not throw. In
particular, \tcode{vector} ([vector]) and \tcode{deque} ([deque]) can be
used.  \begin{note}\tcode{vector<bool>} is not a sequence container.\end{note}
